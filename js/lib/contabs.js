...
Contabs.prototype = {
    ...
    Menu: function(menus) {
        ...
        for (var j = 0; j < menus.length; j++){
            ...
            var firstHtml = '';
            firstHtml += '<div class="nav-item color'+j+'">';
            firstHtml += '<div class="product"><a><h2><span class="fa-stack"><i class="fa fa-circle-o fa-stack-2x"></i><i class="fa '+menus[j].cssClass+' fa-stack-1x"></i></span></h2>'+menus[j].name+'<div class="triangle"></div></a></div>';
            firstHtml += '<div class="product-wrap "><h2><i class="fa '+menus[j].cssClass+'"></i> '+menus[j].name+'</h2><div class="second-menu-list"><ul class="nav j-side-menu js-menu-'+j+'"></ul> </div></div>'
            $('.js-navbar-side').append(firstHtml);

            /*=== 弹开菜单列表 s ===*/
            var secondNav = menus[j].items;
            var html = '';
            for (var i = 0; i < secondNav.length; i++) {
                var tmenu = secondNav[i];
                //默认为active
            ...
            $('.j-side-menu.js-menu-'+j+'').append(html);
            /*=== 弹开菜单列表 e ===*/
